...
export function buildEditTools(ctx: ToolContext, requiresApproval = true) {
  return {
    edit: tool({
      description: `Replace an exact string in a file. Requires read_file on this path first in the current session. This prevents blind edits. \`old_string\` must be unique in the file unless replace_all: true. ${requiresApproval ? "Asks for user approval before writing." : "Runs automatically in autonomous mode."}`,
